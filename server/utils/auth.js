...
if (JWT_SECRET === REFRESH_TOKEN_SECRET) {
  console.warn(
    'SECURITY WARNING: JWT_SECRET and REFRESH_TOKEN_SECRET should not be the same!',
  );
}
...
export const generateTokens = (userId, event = null, rememberMe = false) => {
  ...
  const refreshToken = jwt.sign(refreshTokenPayload, REFRESH_TOKEN_SECRET, {
    expiresIn: Math.floor(
      (rememberMe
        ...
        : REMEMBER_ME_OFF_REFRESH_TOKEN_EXPIRY_MS) / 1000,
    ),
  });
  ...
  console.log(
    ...
    JSON.stringify(decodedRefreshToken, null, 2),
  );

  console.log('Generated tokens:', {
    rememberMe,
    expiresInMs: rememberMe
      ? REMEMBER_ME_ON_REFRESH_TOKEN_EXPIRY_MS
      : REMEMBER_ME_OFF_REFRESH_TOKEN_EXPIRY_MS,
...
export const setAuthCookies = (event, tokens, rememberMe) => {
  ...
  const refreshTokenMaxAgeSeconds =
    (rememberMe
      ? REMEMBER_ME_ON_REFRESH_TOKEN_EXPIRY_MS
  ...
  if (event.context && event.context.auth && event.context.auth.user) {
    try {
      ...
      if (userId) {
        ...
        console.log(
          `Oturum bilgileri kaydedildi: ${userId}, ${clientIp}, ${userAgent?.substring(0, 50)}`,
        );
      }
    } catch (error) {
...
export const isPublicRoute = (path) => {
  const publicPaths = ['/', '/login', '/register', '/glisemik-index'];
...
export const getServerSession = async (event) => {
  ...
  try {
    ...
    console.log(
      '[getServerSession] auth_token from cookie:',
      authToken ? 'found' : 'NOT_FOUND',
    ); // New log

    if (!authToken) {
      console.log(
        '[getServerSession] Auth token (auth_token) not found. Attempting refresh token check.',
      );
      const refreshToken = getCookie(event, 'refresh_token');
      console.log(
        '[getServerSession] refresh_token from cookie:',
        refreshToken ? 'found' : 'NOT_FOUND',
      ); // New log
      if (!refreshToken) {
        console.log(
          '[getServerSession] Refresh token (refresh_token) also not found. Returning null.',
        );
        return null;
      }

      console.log(
        '[getServerSession] Refresh token found, attempting to verify it.',
      );
      const userFromRefreshToken = await verifyRefreshToken(
        refreshToken,
        event,
      ); // Pass event for fingerprinting if verifyRefreshToken uses it
      if (!userFromRefreshToken) {
        console.log(
          '[getServerSession] Refresh token is invalid or user not found. Returning null.',
        );
      ...
      console.log(
        '[getServerSession] Refresh token VALID for user ID:',
      ...
      console.log(
        '[getServerSession] RememberMe value from token:',
        rememberMe,
      );
      ...
      console.log(
        '[getServerSession] New tokens generated and cookies set. Returning session for user ID:',
        userFromRefreshToken._id,
      );
      return {
        user: {
    ...
    console.log(
      '[getServerSession] Auth token (auth_token) found, verifying it.',
    );
    ...
    if (!decoded) {
      if (expired) {
        console.log(
          '[getServerSession] Auth token (auth_token) is EXPIRED. This state should ideally have been caught by the !authToken block above after a page refresh if refresh token logic is primary for session re-establishment without auth_token.',
        );
        ...
        const refreshToken = getCookie(event, 'refresh_token');
        console.log(
          '[getServerSession] Auth token expired, refresh_token from cookie:',
          refreshToken ? 'found' : 'NOT_FOUND',
        );
        if (!refreshToken) {
          console.log(
            '[getServerSession] Auth token expired and Refresh token (refresh_token) also not found. Returning null.',
          );
          return null;
        }
        const userFromRefreshTokenOnExpiry = await verifyRefreshToken(
          refreshToken,
          event,
        );
        if (!userFromRefreshTokenOnExpiry) {
          console.log(
            '[getServerSession] Auth token expired and Refresh token is invalid. Returning null.',
          );
          return null;
        }
        console.log(
          '[getServerSession] Auth token expired. Refresh token VALID for user ID:',
          userFromRefreshTokenOnExpiry._id,
          '. Generating new tokens.',
        );
        const newTokensOnExpiry = generateTokens(
          userFromRefreshTokenOnExpiry._id,
          event,
        );
        setAuthCookies(event, newTokensOnExpiry);
        console.log(
          '[getServerSession] New tokens generated (due to expired auth_token). Returning session for user ID:',
          userFromRefreshTokenOnExpiry._id,
        );
        return {
          user: {
            ...
            lastLogin:
              userFromRefreshTokenOnExpiry.lastLogin ||
              new Date().toISOString(),
          },
        };
      }
      console.log(
        '[getServerSession] Auth token (auth_token) is INVALID (but not expired marker). Returning null.',
      );
    ...
    console.log(
      '[getServerSession] Auth token (auth_token) is VALID and DECODED for userId:',
      decoded.userId,
    );
    const user = await User.findById(decoded.userId).select('-password');
    if (!user) {
      console.log(
        '[getServerSession] Auth token valid, but user ID',
        decoded.userId,
        'not found in DB. Returning null.',
      );
      return null;
    }
    ...
    if (decoded.fingerprint) {
      ...
      console.log(
        '[getServerSession] Token fingerprint check. Decoded:',
        decoded.fingerprint,
        'Current:',
        currentFingerprint,
      );
      if (decoded.fingerprint !== currentFingerprint) {
        throw createError({
          statusCode: 401,
          message: 'Fingerprint mismatch. Possible session hijack attempt.',
        });
    ...
    console.log(
      '[getServerSession] Session fully validated with auth_token. User ID:',
      user._id,
    );
    return {
      user: {
    ...
    console.error(
      '[getServerSession] CRITICAL ERROR in getServerSession:',
      error,
    );
    return null;
  }
...
export const defineAuthenticatedHandler = (handler) => {
  return defineEventHandler(async (event) => {
    ...
    if (!authToken) {
      throw createError({
        statusCode: 401,
        message: 'Access token required (auth_token cookie missing)',
      });
    }
    ...
    try {
      ...
      console.error(
        "Error in authenticated handler's verifyAccessToken call:",
        error,
      );
      throw createError({
        statusCode: 500,
