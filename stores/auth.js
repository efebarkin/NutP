...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    async checkSession() {
      try {
        ...
        if (response?.user) {
          ...
          console.warn('[AuthStore] checkSession: No user data in response from /api/auth/session. Current this.user before clear:', JSON.stringify(this.user), 'Calling clearUser.');
          this.clearUser();
        }
      } catch (error) {
        console.error('[AuthStore] checkSession: Error during /api/auth/session call:', error.data?.message || error.message || error);
        ...
        if (error.data?.statusCode === 401 || error.status === 401 || error.response?.status === 401) {
          console.warn('[AuthStore] checkSession: Caught a 401. This might be after a failed refresh attempt by global handler, or if /api/auth/session is not intercepted. Clearing user.');
          this.clearUser();
        } else {
          // For other types of errors (network, 5xx, etc.)
          console.warn('[AuthStore] checkSession: Caught a non-401 error. Clearing user. Error:', error.data?.message || error.message);
          this.clearUser();
        }
      }
    },
    ...
    async refreshToken() {
      ...
      try {
        console.log('[AuthStore] refreshToken: Attempting to refresh token...');
        ...
        console.log('[AuthStore] refreshToken: /api/auth/refresh responded 2xx. New cookies assumed to be set. Calling checkSession().');
        await this.checkSession(); // This will fetch /api/auth/session and update user state

        if (this.user) {
          console.log('[AuthStore] refreshToken: Session re-validated successfully after refresh. User:', this.user._id);
          ...
          console.warn('[AuthStore] refreshToken: checkSession() did not result in a user after token refresh. Logging out.');
          this.clearUser();
          this.globalLogoutReason = { message: 'Session could not be re-established after token refresh.' };
          return false;
        }

      } catch (error) {
        console.error('[AuthStore] refreshToken: Error during token refresh:', error.data?.message || error.message || error);
        this.clearUser(); // Clear user data on refresh failure
        this.globalLogoutReason = { 
          message: error.data?.message || 'Your session has expired or the refresh attempt failed. Please login again.',
          status: error.status
        };        
        return false;
      } finally {
    ...
    clearUser() {
      ...
      if (import.meta.client && this.fetchInterceptorInstalled && this.originalFetch) {
      ...
      window.fetch = async (...args) => {
        ...
        try {
          ...
          if (response.status === 401) {
            ...
            const isSpecificAuthPath = nonGlobalLogoutPaths.some(path => requestUrl.includes(path));

            if (!isSpecificAuthPath) {
              console.warn(`[AuthStore] Global 401 detected on ${requestUrl}.`);

              if (!this.user || !this.user._id) {
                console.warn(`[AuthStore] Global 401: No user context in store. Cannot attempt refresh. Proceeding to logout.`);
                if (!this.globalLogoutReason) { // Ensure logout reason is set
                    this.clearUser(); // Make sure state is clean
                    this.globalLogoutReason = { message: 'Your session has expired. Please login again.' };
                }
                return response; // Return original 401 response
              }

              console.warn(`[AuthStore] Global 401: User context found (User ID: ${this.user._id}). Attempting token refresh.`);
              try {
                const refreshSuccessful = await this.refreshToken();
                console.warn(`[AuthStore] Global 401 - refreshSuccessful: ${refreshSuccessful}`);

                if (refreshSuccessful) {
                  console.warn(`[AuthStore] Global 401 - Token refresh successful for ${requestUrl}. Retrying original request.`);
                  return await this.originalFetch(...args);
                } else {
                  console.warn(`[AuthStore] Global 401 - Token refresh failed for ${requestUrl}. Original 401 will propagate or logout triggered.`);
                  ...
                  if (!this.globalLogoutReason) {
                     console.warn('[AuthStore] Global 401 - Setting fallback globalLogoutReason and clearing user.');
                     this.clearUser(); 
                     this.globalLogoutReason = { message: 'Your session could not be refreshed. Please login again.' };
                  }
                  return response; // Return original 401 response
                }
              } catch (refreshError) {
                console.warn(`[AuthStore] Global 401 - Error during token refresh attempt for ${requestUrl}:`, refreshError);
                // Ensure user is cleared and logout is signalled if not already handled by refreshToken.
                if (!this.globalLogoutReason) { 
                    this.clearUser(); 
                    this.globalLogoutReason = { message: 'An error occurred while trying to refresh your session. Please login again.' };
                }
                return response; // Return original 401 response
              }
            } else {
              console.warn(`[AuthStore] Global 401 detected on specific auth path ${requestUrl}. Handling as per specific path logic.`);
    ...
    async getSocketToken() {
      try {
        ...
        console.error('Failed to retrieve a valid socket token from the server.');
        this.socketToken = null; // Ensure state is cleared
        return null;

      } catch (error) {
        console.error('Error fetching socket token:', error.data?.message || error.message);
