<template>
  <div
    class="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden"
  >
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      ...
      >
        <div
          class="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center shadow-sm"
        >
          <svg
      ...
      <h2
        class="text-center text-3xl font-bold text-gray-800 animate-fade-in-up animation-delay-100"
      >
        Hesabınıza Giriş Yapın
      </h2>
      <p
        class="mt-2 text-center text-sm text-gray-600 animate-fade-in-up animation-delay-200"
      >
        Hesabınız yok mu?
        <NuxtLink
          to="/register"
    ...
    <div
      class="mt-8 sm:mx-auto sm:w-full sm:max-w-md animate-fade-in-up animation-delay-300"
    >
      <div
        class="bg-white/95 backdrop-blur-sm py-8 px-6 shadow-lg sm:rounded-xl sm:px-10 card-hover border border-gray-100 transition-all duration-300 hover:shadow-xl relative z-10"
      >
        <!-- Error message -->
        <div
          v-if="error"
          class="mb-6 p-3 bg-red-50 text-red-700 rounded-lg flex items-center animate-fade-in"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        >
          ...
          >
            ...
            <div class="relative rounded-md shadow-sm">
              ...
              <p
                v-if="errors.email"
                class="mt-2 text-sm text-red-600 flex items-center absolute bottom-[-24px] left-0 w-full"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-4 w-4 mr-1"
          ...
          >
            ...
            >
              ...
              <a
                href="#"
                class="text-xs font-medium text-green-600 hover:text-green-500 transition-colors flex items-center"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-3 w-3 mr-1"
          ...
          <!-- Submit Button -->
          <div
            class="mt-2 animate-fade-in-up animation-delay-600"
          >
            <button
              type="submit"
  ...
  >
    <div
      class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4 animate-scale-in"
    >
      ...
      <div class="mb-4">
        <p class="text-sm text-gray-600 mb-4">
          Email adresinize gönderilen 6 haneli doğrulama
          kodunu giriniz.
        </p>
...
// Email verification composable with login context
const {
...
// Set up custom success handler for login context
setCustomSuccessHandler(async response => {
  // After email verification in login context, retry the original login
  if (response.user) {
    // The verification already logged us in, handle like normal login success
    ...
    if (redirectParam) {
      await navigateTo(decodeURIComponent(redirectParam));
    } else if (redirectPath) {
      localStorage.removeItem('redirectPath');
      await navigateTo(redirectPath);
    } else {
      await navigateTo(isAdmin ? '/admin' : '/');
    }
  }
});
...
const handleSubmit = validateAndSubmit(async values => {
  ...
  try {
    ...
    console.log(
      '[Login.vue] handleSubmit caught error. Full error object (err):',
      JSON.parse(JSON.stringify(err))
    );
    console.log(
      '[Login.vue] Extracted errorPayload (err.data - should contain a nested .data object):',
      errorPayload
        ? JSON.parse(JSON.stringify(errorPayload))
        : 'undefined'
    );
    ...
    if (
      ...
      errorPayload?.data?.requiresVerification === true
    ) {
      console.log(
        '[Login.vue] Verification condition MET. Displaying verification modal. errorPayload.data:',
        JSON.parse(JSON.stringify(errorPayload.data))
      );
      // Show verification modal using composable
      openVerificationModal(
        errorPayload.data.userId,
        errorPayload.data.email
      );
      ...
      console.log(
        '[Login.vue] Verification condition NOT MET. Handling as other error. errorPayload:',
        errorPayload
          ? JSON.parse(JSON.stringify(errorPayload))
          : 'undefined'
      );
      ...
      if (errorPayload?.data) {
        // If there's a nested 'data' object, it might contain specific field errors (e.g. Zod)
        Object.keys(errorPayload.data).forEach(key => {
          if (errors.hasOwnProperty(key)) {
            // Ensure 'errors' from useForm has this key
            // This assumes Zod-like error structure: { field: { _errors: ['message'] } }
            errors[key] =
              errorPayload.data[key]?._errors?.[0] || '';
          }
        });
      }
      ...
      error.value =
        ...
        (err &&
          err.message) /* fallback for unexpected structures */ ||
        'Giriş yapılırken bir hata oluştu.';
      console.log(
        '[Login.vue] General error.value set to:',
        error.value
      );
    }
  }
