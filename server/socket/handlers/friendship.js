    ...
    get_friends: async (data, callback) => {
      try {
        if (!userId) {
          ...
          return callback({ success: false, error: 'Kullanıcı kimliği bulunamadı' });
        }
    ...
    get_friend_requests: async (data, callback) => {
      try {
        ...
        const requestDetails = requesters.map(requester => {
          const request = requests.find(r => r.requester.toString() === requester._id.toString());
          return {
            ...requester,
        ...
        console.log(`Found ${requestDetails.length} friend requests for user: ${userId}`);
    ...
    get_sent_requests: async (data, callback) => {
      try {
        ...
        const requestDetails = recipients.map(recipient => {
          const request = sentRequests.find(r => r.recipient.toString() === recipient._id.toString());
          return {
            ...recipient,
            requestId: request._id,
            createdAt: request.createdAt
        ...
        console.log(`Found ${requestDetails.length} sent requests for user: ${userId}`);
        callback({ success: true, data: requestDetails });
      } catch (error) {
    ...
    send_friend_request: async (data, callback) => {
      try {
        ...
        if (recipientId === userId) {
          ...
          return callback({ success: false, error: 'Kendinize arkadaşlık isteği gönderemezsiniz' });
        }
        
        console.log(`Arkadaşlık isteği gönderiliyor: ${userId} -> ${recipientId}`);
        
        // Kullanıcının varlığını kontrol et
        const recipient = await User.findById(recipientId);
        if (!recipient) {
          console.error(`Alıcı kullanıcı bulunamadı: ${recipientId}`);
          return callback({ success: false, error: 'Alıcı kullanıcı bulunamadı' });
        }
        ...
        if (existingFriendship) {
          ...
          if (existingFriendship.status === 'rejected') {
            ...
            console.log('Reddedilmiş arkadaşlık isteği güncellendi:', updatedFriendship._id);
            ...
            if (recipientSocket) {
              io.to(recipientSocket).emit('friend_request_received', {
                ...
                requester: await User.findById(userId, { password: 0, email: 0 }).lean()
              });
            }
        ...
        const recipientSocket = connectedUsers.get(recipientId.toString());
        if (recipientSocket) {
          const requesterInfo = await User.findById(userId, { password: 0, email: 0 }).lean();
          io.to(recipientSocket).emit('friend_request_received', {
    ...
    accept_friend_request: async (data, callback) => {
      try {
        ...
        if (!friendRequest) {
          ...
          return callback({ success: false, error: 'Arkadaşlık isteği bulunamadı' });
        }
        
        // İsteğin bu kullanıcıya ait olduğunu kontrol et
        if (friendRequest.recipient.toString() !== userId.toString()) {
          console.error('Bu arkadaşlık isteğini kabul etme yetkiniz yok');
          return callback({ success: false, error: 'Bu arkadaşlık isteğini kabul etme yetkiniz yok' });
        }
        ...
        const requester = await User.findById(friendRequest.requester, { password: 0, email: 0 }).lean();
        const recipient = await User.findById(friendRequest.recipient, { password: 0, email: 0 }).lean();
        
        if (!requester || !recipient) {
          console.error('Kullanıcı bilgileri bulunamadı');
          return callback({ 
            success: true, 
            message: 'Arkadaşlık isteği kabul edildi, ancak kullanıcı bilgileri alınamadı',
        ...
        console.log('Bağlı kullanıcılar:', Array.from(connectedUsers.entries()));
        
        // İstek gönderene bildirim gönder
        const requesterSocket = connectedUsers.get(friendRequest.requester.toString());
        if (requesterSocket) {
          console.log('İstek gönderene bildirim gönderiliyor:', friendRequest.requester.toString());
          io.to(requesterSocket).emit('friend_request_accepted', {
            requestId: friendRequest._id,
            friend: recipientWithStatus
          });
        } else {
          console.log('İstek gönderen bağlı değil:', friendRequest.requester.toString());
        }
        
        // Alıcıya da (kabul eden kişiye) bildirim gönder
        const recipientSocket = connectedUsers.get(friendRequest.recipient.toString());
        if (recipientSocket) {
          console.log('Alıcıya bildirim gönderiliyor:', friendRequest.recipient.toString());
    ...
    reject_friend_request: async (data, callback) => {
      try {
        ...
        if (request.recipient.toString() !== userId) {
          return callback({ success: false, error: 'You are not authorized to reject this request' });
        }

        if (request.status !== 'pending') {
          return callback({ success: false, error: 'This request is not pending' });
        }
        ...
        // İstek gönderene bildirim gönder
        const requesterSocketId = connectedUsers.get(request.requester.toString());
    ...
    cancel_friend_request: async (data, callback) => {
      try {
        ...
        if (request.requester.toString() !== userId) {
          return callback({ success: false, error: 'You are not authorized to cancel this request' });
        }
        ...
        // Alıcıya bildirim gönder
        const recipientSocketId = connectedUsers.get(request.recipient.toString());
