...
import { getUserConversationsKey, delAsync, getUserOnlineStatus } from '../../utils/redis';
  ...
  return {
        ...
        const conversationsWithOnlineStatus = await Promise.all(
          conversations.map(async (conversation) => {
            ...
            if (otherParticipant) {
              ...
              const isConnected = connectedUsers.has(otherParticipant._id.toString());
              // Redis'ten çevrimiçi durumunu kontrol et
              const status = await getUserOnlineStatus(otherParticipant._id.toString());
    ...
    create_conversation: async (data, callback) => {
      try {
        ...
        if (!participantId) {
          return callback({ success: false, error: 'Participant ID is required' });
        }
        
        // Servis kullanarak konuşma oluştur veya mevcut konuşmayı getir
        const conversation = await conversationService.getOrCreateConversation(userId, participantId);
        
        // Konuşmayı katılımcı detaylarıyla birlikte getir
        const populatedConversation = await conversationService.getConversationById(conversation._id, userId);
        ...
        const conversationWithOnlineStatus = {
          ...
          participants: await Promise.all(
            populatedConversation.participants.map(async (participant) => {
              // Socket.io bağlantısı varsa çevrimiçi
              const isConnected = connectedUsers.has(participant._id.toString());
              // Redis'ten çevrimiçi durumunu kontrol et
              const status = await getUserOnlineStatus(participant._id.toString());
        ...
        callback({ 
          ...
          message: conversation.isNew ? 'New conversation created' : 'Existing conversation found'
        });
        
        // Diğer katılımcıya bildirim gönder
        const participantSocketId = connectedUsers.get(participantId);
        if (participantSocketId) {
          io.to(participantSocketId).emit('new_conversation', conversationWithOnlineStatus);
        }
    ...
    archive_conversation: async (data, callback) => {
      try {
        ...
        callback({ success: true, message: 'Conversation archived successfully' });
        ...
        if (conversation) {
          conversation.participants.forEach(participant => {
            if (participant._id.toString() !== userId) {
              ...
              if (participantSocketId) {
                io.to(participantSocketId).emit('conversation_archived', { conversationId });
              }
            }
          });
        }
