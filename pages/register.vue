    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            class="space-y-3 bg-gray-50 p-4 rounded-xl border border-gray-100 animate-fade-in-up animation-delay-800"
          >
            <p
            ...
            <ul class="text-sm space-y-2 mt-2">
              ...
              >
                <span
                  class="flex-shrink-0 w-5 h-5 mr-2 flex items-center justify-center rounded-full"
                  :class="
                    passwordRequirements.minLength
                      ? 'bg-green-100'
                      : 'bg-gray-100'
              ...
              >
                ...
                >
                  <svg
                    v-if="passwordRequirements.hasLowerCase"
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-3.5 w-3.5"
                    fill="none"
                    viewBox="0 0 24 24"
          ...
          >
            ...
            >
              ...
              ></span>
              <span v-if="!submitting" class="relative"
                >Kayıt Ol</span
              >
            </button>
          </div>
...
const { value: passwordConfirm } = useField(
  'passwordConfirm'
);
...
// Email verification composable with register context
const {
  ...
  try {
    ...
    if (result && result.success) {
      if (result.requiresVerification) {
        // Show email verification modal using composable
        openVerificationModal(result.userId, result.email);
        toast.info(
        ...
        toast.success(
          'Hesabınız başarılı bir şekilde oluşturuldu ve otomatik giriş yapıldı'
        );

        // Kullanıcı rolüne göre yönlendirme yap
        if (
          authStore.user &&
          Array.isArray(authStore.user.role)
        ) {
