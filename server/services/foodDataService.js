    ...
    try {
      ...
      while (hasMorePages) {
        ...
        await Food.insertMany(foods, { ordered: false }).catch((err) => {
          console.log(
            `Some foods in page ${pageNumber} already exist, continuing...`,
          );
        });

        totalFoods += foods.length;
        console.log(
          `Saved ${foods.length} foods from page ${pageNumber}. Total foods saved: ${totalFoods}`,
        );
